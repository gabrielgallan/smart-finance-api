...
describe('Get account summaries by categories use case', () => {
  ...
  it('should be able to get account summaries by categories', async () => {
    ...
    if (result.isRight()) {
      expect(result.value.accountSummariesByCategories).toHaveLength(2)
      expect(
        result.value.accountSummariesByCategories[0].accountSummary.netBalance,
      ).toBe(100)
      expect(result.value.accountSummariesByCategories[0].category.name).toBe(
        'Freelance jobs',
      )
      expect(
        result.value.accountSummariesByCategories[1].accountSummary.netBalance,
      ).toBe(-75)
      expect(result.value.accountSummariesByCategories[1].category.name).toBe(
        'Sport expenses',
      )
    }
  })
